        ...
        oaicompat_type oaicompat) -> void {
            ...
            if (!stream) {
                ...
                const auto chunked_content_provider = [id_task, active_task_id, &ctx_server, completion_id, oaicompat, send_done = params.send_done, data](size_t, httplib::DataSink& sink) mutable {
                    ...
                    // 1. Parse banned strings from request
                    ...
                    if (data.contains("banned_strings") && data["banned_strings"].is_array()) {
                        for (const auto& val : data["banned_strings"]) {
                            if (val.is_string()) {
                                stop_phrases.push_back(val.get<std::string>());
                            }
                        }
                    }

                    // 2. Calculate Buffer Size dynamically based on CHARACTER length
                    ...
                    // Set buffer size:
                    // - Minimum 6 (for efficiency)
                    // - Or max_banned_char_len + 2 (safety margin)
                    // Using character length as token buffer size is a safe upper bound.
                    const size_t BUFFER_SIZE = std::max((size_t)6, max_banned_char_len + 2);
                            ...
                            if (found_split) {
                                ...
                                if (guilty_token_id != -1) {
                                    ...
                                    for (int banned_id : current_step_bans) {
                                        fix_data["logit_bias"].push_back(json::array({banned_id, -999.0f}));
                                    }
